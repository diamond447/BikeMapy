...
| Frontend lint, types, tests, build | `cd frontend && pnpm lint && pnpm format && pnpm exec tsc -b --pretty false && pnpm test && VITE_PUBLIC_SITE_URL=http://localhost:5173 VITE_API_URL=http://localhost:8000 VITE_ENABLE_REPORTS=false pnpm build`; inspect the generated bundle. | Run locally for this change; owner/CI evidence remains separate. |
