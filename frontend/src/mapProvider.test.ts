...
describe('map provider attribution', () => {
  it('uses clickable official attribution links by default', () => {
    ...
    expect(MAP_PROVIDER.attribution).toContain('href="https://www.openmaptiles.org/"')
    expect(MAP_PROVIDER.attribution).toContain('href="https://www.openstreetmap.org/copyright"')
    expect(MAP_PROVIDER.attribution).toContain('>OpenStreetMap</a>')
    expect(MAP_PROVIDER.attribution).toContain('Data from')
  })
})
